...
We've added several custom `_includes` to provide users with more customization options for different site elements. We've also added a section to [Configuration]({% link docs/Usage Guidelines/customization.md %}#override-includes) to outline these.
...
For more, see [Custom layouts and includes]({% link docs/Usage Guidelines/customization.md %}#custom-layouts-and-includes). PR: [#1058].
...
In addition to customizing the search placeholder, we've also added the ability to provide custom content to the search index. for more, see [Custom content for search index]({% link docs/Usage Guidelines/search.md %}#custom-content-for-search-index). PR: [#1068].
...
- Links wrap. PR: [#905]
- Callouts: a new component similar to alerts or banners. See [UI Components - Callouts]({% link docs/Usage Guidelines/ui-components/callouts.md %}). PR: [#466]
